	...
	)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			{
				weight_max=alpha * T;
				idx_max = collected_id[j]; //记录最大权重的高斯id
				flag_update = 1;
			}
...
template <uint32_t CHANNELS>
__global__ void __launch_bounds__(BLOCK_X * BLOCK_Y) //改进的渲染
render_depthCUDA(
	const uint2* __restrict__ ranges,
	const uint32_t* __restrict__ point_list,
	int W, int H,
	...
	)
{
	...
	//从屏幕坐标生成射线，并将其转换为世界坐标
    glm::mat4 matrix = glm::make_mat4x4(projmatrix);
    glm::mat4 matrix_temp = glm::inverse(matrix);
	float *projmatrix_inv= glm::value_ptr(matrix_temp); //指针

	glm::vec3 ray_origin = *cam_pos;  //射线的起点
	...
	float3 p_proj_r = { Pix2ndc(pixf.x, W), Pix2ndc(pixf.y, H), 1}; //将像素坐标转换为归一化设备坐标
	...
	glm::vec3 ray_direction={ //利用像素位置和相机位置计算射线方向
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			float discriminant=b*b-4*a*c;	//二元一次方程的判别式，具体计算可参考附录
